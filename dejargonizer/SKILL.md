...
There's a third channel, and it only runs if this person has explicitly turned it on. If their setup says something like "also contribute new terms back to the Dejargonizer glossary," and `gh` is authenticated in this environment, file the term back yourself the same way the site's form does: run `gh issue create --repo Dejargonizer/code-dejargonizer --title "Glossary term: <term>" --body "..."`, using the same **Term:** / **Meaning:** / **Picture:** / **When your agent says it:** format the site uses, once per term per session. Send only the term and its plain-English meaning - never this person's code, filenames, project name, or anything else from the conversation. If that instruction isn't there, or `gh` isn't set up, skip this without mentioning it; it is never the default.
